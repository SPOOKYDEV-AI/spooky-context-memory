...
describe("model-based persistence gauntlet", () => {
  it(`matches a reference counter across ${scenarios} generated state-machine scenarios`, async () => {
        ...
        if (action < 60) {
          ...
          const persisted = await journal.append(
            ...
            amounts.map((amount, index) => ({
              ...
              payload: { amount },
              schemaVersion: 1,
              occurredAt: `2026-07-31T00:${String(scenario).padStart(2, "0")}:${String(step).padStart(2, "0")}.${String(index).padStart(3, "0")}Z`,
            })),
          ...
          await expect(
            journal.append(
              ...
              { expectedSequence: modelSequence === 0 ? 1 : modelSequence - 1 },
            ),
